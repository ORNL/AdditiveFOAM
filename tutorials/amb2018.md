...
- Optional refinement and runtime redistribution configuration; both are disabled by default.
...
| `constant/dynamicMeshDict` | OpenFOAM topology changer and cell-count-based mesh distributor, with its type set to `none` by default |
...
| `system/decomposeParDict` | Eight-rank initial decomposition, optional PT-Scotch redistribution, and refinement-history constraint |
...
The case includes a PT-Scotch runtime-redistribution configuration with a 10-time-step check interval, a 10% cell-count imbalance threshold, and the required refinement-history constraint. Redistribution is disabled by `type none` in `constant/dynamicMeshDict`; change this to `type distributor`, then select an active refinement model as described in the [AMR guide]({{ '/docs/amr/' | relative_url }}) to exercise refinement and redistribution.
