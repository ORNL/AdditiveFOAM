...
heat-source calibration campaign. It uses a `projected` heat source with a
...
- `constant/heatSourceDict`: `<<nIntercept>>`, `<<D4Sigma>>`, `<<isotherm>>`
- `constant/scanPath`: `<<power>>`, `<<velocity>>`
- `system/controlDict`: `<<endTime>>`, `<<writeInterval>>`, `<<isotherm>>`
...
integral and D4Sigma during initialization and uses `depth` as the configured
projected source depth.
...
Marangoni coefficient. The calibration command resolves the selected isotherm
from that material configuration and renders it consistently into both users.
